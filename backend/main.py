"""FastAPI backend for DM Debate Studio."""
...
app = FastAPI(title="DM Debate Studio API")
...
@app.get("/")
async def root():
    return {"status": "ok", "service": "DM Debate Studio API"}
...
async def list_conversations(user: AuthUser = Depends(get_current_user)):
    return storage.list_conversations(user.uid)
...
async def create_conversation(
    ...
    conversation = storage.create_conversation(user.uid, conversation_id)
    return conversation
...
async def get_conversation(
    ...
    conversation = storage.get_conversation(user.uid, conversation_id)
...
async def rename_conversation(
    ...
    if conversation is None:
        raise HTTPException(status_code=404, detail="Conversation not found")
    storage.update_conversation_title(user.uid, conversation_id, body.title)
...
async def delete_conversation(
    ...
    campaign_lookup = campaigns.lookup_campaign_for_conversation(user.uid, conversation_id)
    if campaign_lookup:
    ...
    deleted = storage.delete_conversation(user.uid, conversation_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def send_message_stream(
    ...
    async def event_generator():
        try:
            ...
            storage.add_user_message(uid, conversation_id, body.content)
            ...
            if not any(r for r in stage1_results if not r.get("error")):
                ...
                storage.add_error_message(uid, conversation_id, error_msg)
            ...
            storage.add_error_message(uid, conversation_id, f"Error: {str(e)}")
...
async def debate_stream(
    ...
    file_metadatas = []
    if body.file_ids:
        from .uploads import _uploads_dir, IMAGE_EXTENSIONS, DOCUMENT_EXTENSIONS, MIME_MAP

        conv_upload_dir = _uploads_dir(uid, conversation_id)
    ...
    async def event_generator():
        try:
            storage.add_user_message(
                ...
                conversation_id,
                body.content,
                file_metadatas=file_metadatas if file_metadatas else None,
            )
            ...
            campaign_context = ""
            campaign_lookup = campaigns.lookup_campaign_for_conversation(uid, conversation_id)
            if campaign_lookup:
                campaign_context = campaigns.get_stage_context(
                    uid, campaign_lookup["campaign_id"], campaign_lookup["stage_id"]
                )
                sources_context = campaigns.get_sources_context(uid, campaign_lookup["campaign_id"])
                if sources_context:
                    campaign_context = (
                        campaign_context + "\n\n" + sources_context
                        if campaign_context
                        else sources_context
                    )
            ...
            if chat_response_data:
                storage.add_chat_message(uid, conversation_id, chat_response_data, metadata if metadata else None)
            else:
                storage.add_debate_message(uid, conversation_id, debate_entries, summary_data, metadata if metadata else None)
...
async def interject_in_debate(
    conversation_id: str,
    body: InterjectionRequest,
    user: AuthUser = Depends(get_current_user),
):
    conversation = storage.get_conversation(user.uid, conversation_id)
    if conversation is None:
...
async def create_campaign(body: CreateCampaignRequest, user: AuthUser = Depends(get_current_user)):
    return campaigns.create_campaign(user.uid, body.name, description=body.description or "")


@app.get("/api/campaigns")
async def list_all_campaigns(user: AuthUser = Depends(get_current_user)):
    return campaigns.list_campaigns(user.uid)


@app.get("/api/campaigns/{campaign_id}")
async def get_campaign(campaign_id: str, user: AuthUser = Depends(get_current_user)):
    campaign = campaigns.get_campaign(user.uid, campaign_id)
    if campaign is None:
...
async def update_campaign(campaign_id: str, body: UpdateCampaignRequest, user: AuthUser = Depends(get_current_user)):
    result = campaigns.update_campaign(user.uid, campaign_id, name=body.name, description=body.description)
    if result is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    return result
...
async def delete_campaign(campaign_id: str, user: AuthUser = Depends(get_current_user)):
    deleted = campaigns.delete_campaign(user.uid, campaign_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Campaign not found")
    return {"status": "deleted"}
...
async def add_campaign_stage(campaign_id: str, body: AddStageRequest, user: AuthUser = Depends(get_current_user)):
    stage = campaigns.add_stage(user.uid, campaign_id, body.name)
    if stage is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    return stage
...
async def reorder_campaign_stages(campaign_id: str, body: ReorderStagesRequest, user: AuthUser = Depends(get_current_user)):
    result = campaigns.reorder_stages(user.uid, campaign_id, body.stage_ids)
    if result is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    return result
...
async def update_campaign_stage(campaign_id: str, stage_id: str, body: UpdateStageRequest, user: AuthUser = Depends(get_current_user)):
    updates = {}
    if body.name is not None:
        updates["name"] = body.name
    if body.status is not None:
        updates["status"] = body.status
    result = campaigns.update_stage(user.uid, campaign_id, stage_id, updates)
    if result is None:
...
async def delete_campaign_stage(campaign_id: str, stage_id: str, user: AuthUser = Depends(get_current_user)):
    deleted = campaigns.delete_stage(user.uid, campaign_id, stage_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Campaign or stage not found")
    return {"status": "deleted"}
...
async def add_stage_conversation(campaign_id: str, stage_id: str, user: AuthUser = Depends(get_current_user)):
    result = campaigns.add_conversation_to_stage(user.uid, campaign_id, stage_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Campaign or stage not found")
    return result
...
async def create_text_source(campaign_id: str, body: TextSourceBody, user: AuthUser = Depends(get_current_user)):
    try:
        meta = campaigns.add_text_source(user.uid, campaign_id, body.name, body.content)
        if meta is None:
            raise HTTPException(status_code=404, detail="Campaign not found")
        return meta
...
async def upload_campaign_source(campaign_id: str, file: UploadFile = File(...), user: AuthUser = Depends(get_current_user)):
    try:
        meta = await campaigns.add_source(user.uid, campaign_id, file)
        if meta is None:
            raise HTTPException(status_code=404, detail="Campaign not found")
        return meta
    except ValueError as e:
...
async def list_campaign_sources(campaign_id: str, user: AuthUser = Depends(get_current_user)):
    sources = campaigns.list_sources(user.uid, campaign_id)
    if sources is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    return sources
...
async def rename_campaign_source(campaign_id: str, source_id: str, body: RenameSourceBody, user: AuthUser = Depends(get_current_user)):
    if not body.name.strip():
        raise HTTPException(status_code=400, detail="Name cannot be empty")
    result = campaigns.rename_source(user.uid, campaign_id, source_id, body.name)
    if result is None:
...
async def delete_campaign_source(campaign_id: str, source_id: str, user: AuthUser = Depends(get_current_user)):
    deleted = campaigns.delete_source(user.uid, campaign_id, source_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Source not found")
    return {"status": "deleted"}
...
async def serve_campaign_source(campaign_id: str, source_id: str, user: AuthUser = Depends(get_current_user)):
    filepath = campaigns.get_source_path(user.uid, campaign_id, source_id)
    if filepath is None:
        raise HTTPException(status_code=404, detail="Source not found")
...
async def get_stage_debate_config(campaign_id: str, stage_id: str, user: AuthUser = Depends(get_current_user)):
    campaign = campaigns.get_campaign(user.uid, campaign_id)
    if campaign is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
    cfg = campaigns.get_stage_debate_config(user.uid, campaign_id, stage_id)
    return {"debate_config": cfg}


@app.put("/api/campaigns/{campaign_id}/stages/{stage_id}/debate-config")
async def set_stage_debate_config(campaign_id: str, stage_id: str, body: DebateConfigBody, user: AuthUser = Depends(get_current_user)):
    result = campaigns.update_stage(
        user.uid, campaign_id, stage_id,
...
async def clear_stage_debate_config(campaign_id: str, stage_id: str, user: AuthUser = Depends(get_current_user)):
    result = campaigns.update_stage(user.uid, campaign_id, stage_id, {"debate_config": None})
    if result is None:
        raise HTTPException(status_code=404, detail="Campaign or stage not found")
...
async def get_conversation_debate_config(conversation_id: str, user: AuthUser = Depends(get_current_user)):
    conversation = storage.get_conversation(user.uid, conversation_id)
    if conversation is None:
        raise HTTPException(status_code=404, detail="Conversation not found")
    cfg = storage.get_conversation_debate_config(user.uid, conversation_id)
    return {"debate_config": cfg}


@app.put("/api/conversations/{conversation_id}/debate-config")
async def set_conversation_debate_config(conversation_id: str, body: DebateConfigBody, user: AuthUser = Depends(get_current_user)):
    try:
...
async def clear_conversation_debate_config(conversation_id: str, user: AuthUser = Depends(get_current_user)):
    try:
        storage.update_conversation_debate_config(user.uid, conversation_id, None)
    except ValueError:
        raise HTTPException(status_code=404, detail="Conversation not found")
    return {"status": "cleared"}
...
async def upload_file(conversation_id: str, file: UploadFile = File(...), user: AuthUser = Depends(get_current_user)):
    conversation = storage.get_conversation(user.uid, conversation_id)
    if conversation is None:
        raise HTTPException(status_code=404, detail="Conversation not found")
    try:
        metadata = await save_upload(user.uid, conversation_id, file)
        return metadata
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def serve_file(conversation_id: str, filename: str, user: AuthUser = Depends(get_current_user)):
    filepath = get_upload_path(user.uid, conversation_id, filename)
    if filepath is None:
...
async def update_app_settings(request: UpdateSettingsRequest, user: AuthUser = Depends(get_current_user)):
    updates = {}
    ...
    if request.search_provider is not None:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid search provider. Must be one of: {[p.value for p in SearchProvider]}")

    if request.search_keyword_extraction is not None:
        if request.search_keyword_extraction not in ["direct", "yake"]:
            raise HTTPException(status_code=400, detail="Invalid keyword extraction mode.")
...
async def test_tavily_api(request: TestTavilyRequest, user: AuthUser = Depends(get_current_user)):
    import httpx
    settings = get_settings()
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.post(
                ...
                json={"api_key": request.api_key or settings.tavily_api_key, "query": "test", "max_results": 1, "search_depth": "basic"},
            )
...
async def test_brave_api(request: TestBraveRequest, user: AuthUser = Depends(get_current_user)):
    import httpx
    settings = get_settings()
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.get(
                ...
                headers={"Accept": "application/json", "Accept-Encoding": "gzip", "X-Subscription-Token": request.api_key or settings.brave_api_key},
            )
            if response.status_code == 200:
                return {"success": True, "message": "API key is valid"}
...
async def test_serper_api(request: TestSerperRequest, user: AuthUser = Depends(get_current_user)):
    import httpx
    settings = get_settings()
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.post(
                ...
                json={"q": "test", "num": 1},
                headers={"X-API-KEY": request.api_key or settings.serper_api_key, "Content-Type": "application/json"},
            )
            if response.status_code == 200:
                return {"success": True, "message": "API key is valid"}
            elif response.status_code in (401, 403):
...
async def test_openrouter_api(request: TestOpenRouterRequest, user: AuthUser = Depends(get_current_user)):
    import httpx
...
async def test_provider_api(request: TestProviderRequest, user: AuthUser = Depends(get_current_user)):
...
async def test_ollama_connection(request: TestOllamaRequest, user: AuthUser = Depends(get_current_user)):
    import httpx
...
async def get_ollama_tags(base_url: Optional[str] = None, user: AuthUser = Depends(get_current_user)):
    import httpx
...
async def test_custom_endpoint(request: TestCustomEndpointRequest, user: AuthUser = Depends(get_current_user)):
...
async def publish_conversation(
    ...
    """Publish a conversation to the shared team feed."""
    conversation = storage.get_conversation(user.uid, conversation_id)
    if conversation is None:
        raise HTTPException(status_code=404, detail="Conversation not found")

    db = get_db()
    published_id = uuid.uuid4().hex[:12]
    ...
    published_item = {
        ...
        "itemType": _detect_item_type(conversation),
        "dismissedBy": [],
    }

    db.collection("shared").document("published").collection("items").document(published_id).set(published_item)

    conv_ref = storage._conv_ref(user.uid, conversation_id)
    conv_ref.update({"published": True, "publishedItemId": published_id})

    return {"status": "published", "publishedItemId": published_id}


@app.get("/api/team-feed")
async def get_team_feed(user: AuthUser = Depends(get_current_user)):
    """Get all published items visible to the current user."""
    ...
    docs = (
        ...
        .document("published")
        .collection("items")
        .order_by("publishedAt", direction=gc_firestore.Query.DESCENDING)
        .stream()
    )
    ...
    for doc in docs:
        data = doc.to_dict()
        if user.uid in data.get("dismissedBy", []):
            continue
        items.append({
            "id": data["id"],
            "userId": data["userId"],
            ...
            "itemType": data.get("itemType", "debate"),
            "isOwner": data["userId"] == user.uid,
            "messageCount": len(data.get("conversationSnapshot", {}).get("messages", [])),
        })

    return items


@app.get("/api/team-feed/{item_id}")
async def get_published_item(item_id: str, user: AuthUser = Depends(get_current_user)):
    """Get a single published item with full conversation snapshot."""
    db = get_db()
    doc = db.collection("shared").document("published").collection("items").document(item_id).get()
    if not doc.exists:
    ...
    return {
        ...
        "itemType": data.get("itemType", "debate"),
        "isOwner": data["userId"] == user.uid,
        "conversationSnapshot": data.get("conversationSnapshot", {}),
    }


@app.delete("/api/team-feed/{item_id}")
async def unpublish_item(item_id: str, user: AuthUser = Depends(get_current_user)):
    """Unpublish an item (only the author can do this)."""
    db = get_db()
    ref = db.collection("shared").document("published").collection("items").document(item_id)
    doc = ref.get()
    if not doc.exists:
        raise HTTPException(status_code=404, detail="Published item not found")
    ...
    if data["userId"] != user.uid:
        raise HTTPException(status_code=403, detail="Only the author can unpublish")

    ref.delete()
    return {"status": "unpublished"}


@app.post("/api/team-feed/{item_id}/dismiss")
async def dismiss_item(item_id: str, user: AuthUser = Depends(get_current_user)):
    """Dismiss an item from the current user's feed."""
    from google.cloud import firestore as gc_firestore
    ...
    ref.update({"dismissedBy": gc_firestore.ArrayUnion([user.uid])})
...
@app.get("/api/team-feed/{item_id}/comments")
async def get_comments(item_id: str, user: AuthUser = Depends(get_current_user)):
    """Get comments for a published item."""
    from google.cloud import firestore as gc_firestore
    ...
    docs = (
        ...
        .document(item_id)
        .collection("comments")
        .order_by("createdAt", direction=gc_firestore.Query.ASCENDING)
        .stream()
    )
...
@app.post("/api/team-feed/{item_id}/comments")
async def add_comment(item_id: str, body: CommentRequest, user: AuthUser = Depends(get_current_user)):
    """Add a comment to a published item."""
    db = get_db()
    item_ref = db.collection("shared").document("published").collection("items").document(item_id)
    ...
    item_ref.collection("comments").document(comment_id).set(comment)

    return {"id": comment_id, **comment, "isOwner": True}


@app.delete("/api/team-feed/{item_id}/comments/{comment_id}")
async def delete_comment(item_id: str, comment_id: str, user: AuthUser = Depends(get_current_user)):
...
def _detect_item_type(conversation: Dict[str, Any]) -> str:
    """Detect whether a conversation is council, debate, or chat."""
    for msg in reversed(conversation.get("messages", [])):
        if msg.get("role") != "assistant":
            continue
        if msg.get("mode") == "debate":
            return "debate"
        if msg.get("mode") == "chat":
            return "chat"
        if msg.get("stage1"):
            return "council"
    return "debate"
